...
# Model ID khi gọi qua OpenRouter (ưu tiên nếu có OPENROUTER_API_KEY — có model ":free")
OPENROUTER_MODEL = "openai/gpt-4o-mini"
# Model ID khi gọi trực tiếp OpenAI (fallback nếu chỉ có OPENAI_API_KEY)
...
# =============================================================================
# LLM CLIENT — OpenRouter (ưu tiên, có model :free) hoặc OpenAI trực tiếp (fallback)
# =============================================================================

def _get_llm_client_and_model():
    """
    Chọn client + model theo API key nào có sẵn trong .env:
        - OPENROUTER_API_KEY (ưu tiên): base_url OpenRouter, model id dạng "provider/model"
        - OPENAI_API_KEY (fallback): OpenAI trực tiếp, model id chuẩn OpenAI
    """
    ...
    raise RuntimeError(
        "Cần OPENROUTER_API_KEY hoặc OPENAI_API_KEY trong .env để gọi LLM sinh câu trả lời."
    )
